...
int main(int argc, const char *argv)
{
	...
	if ((ret = test_cipher("ecb(kuznyechik)", kuznyechik_ecb_ct, NULL, 0)) != 0)
		goto out;

	if ((ret = test_cipher("ecb(magma)", magma_ecb_ct, NULL, 0)) != 0)
		goto out;

	/**********************************************************************/

	uint8_t kuznyechik_cbc_ct[48] = {
		0x47, 0x01, 0x08, 0xc9, 0x0e, 0x9b, 0xda, 0xf6,
		0x0c, 0xc7, 0x44, 0x6b, 0x5b, 0xf3, 0xc3, 0x9b,
		0x85, 0xe5, 0x37, 0x7c, 0x2b, 0x96, 0xab, 0xe2,
		0x65, 0xaa, 0x17, 0x29, 0x58, 0x01, 0x9d, 0x02,
		0x7d, 0x67, 0x46, 0x21, 0x72, 0x41, 0x81, 0xef,
		0x84, 0x12, 0xed, 0x27, 0x74, 0xd2, 0xec, 0x7d
	};

	uint8_t magma_cbc_ct[48] = {
		0xea, 0x1f, 0xbc, 0xa6, 0xa8, 0x00, 0x9a, 0xbf,
		0xb7, 0x3e, 0x10, 0xfc, 0x00, 0x7c, 0x29, 0x30,
		0x83, 0x70, 0xe4, 0x69, 0xc0, 0x75, 0x69, 0x0b,
		0xb2, 0x96, 0x5e, 0xfc, 0x51, 0xb2, 0xfc, 0xea,
		0x34, 0x68, 0x89, 0x8d, 0x5e, 0x4f, 0xb9, 0x56,
		0x06, 0x86, 0xa8, 0xbb, 0xa5, 0x94, 0xed, 0xf2
	};

	if ((ret = test_cipher("cbc(kuznyechik)", kuznyechik_cbc_ct, test_iv, 16)) != 0)
		goto out;

	if ((ret = test_cipher("cbc(magma)", magma_cbc_ct, test_iv, 8)) != 0)
		goto out;
